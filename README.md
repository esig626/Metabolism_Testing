...
The methodological question is whether these observable distribution families can be discriminated with controlled finite-sample error, including settings where conventional flux estimation is non-identifiable or unstable.
...
- [`simulation/`](simulation/) contains the scoped FluxEMU simulation and statistical-testing snapshot, with provenance, compact fixtures and tests.
- [`audit/`](audit/) contains search logs, screening ledgers, decisions, raw search captures, citation checks, verification artifacts, deduplication records, red-team material and other review provenance.
